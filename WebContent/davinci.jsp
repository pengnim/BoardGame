...
var bl4=["b13","b14","b15","b16","b17","b18","b19","b20","b21","b22","b23","b24"];
...
function pagestart() {
	...
	ResultSet rs=DBUtil.getGrpMem(con,grp,3);
	...
	while(rs.next()){
		...
		if(DBUtil.checkDPlayer(con,pl[0])||DBUtil.checkDPlayer(con,pl[1])||DBUtil.checkDPlayer(con, pl[2])){
			out.print("document.getElementById('grp').value='-1';");
			out.print("document.Ent.submit();");
		}
		System.out.println(pl[0]+pl[1]+pl[2]);
		...
		ResultSet dc=DBUtil.findDeq3Col(con, pl[0]);
		ResultSet dn=DBUtil.findDeq3Num(con,pl[0]);
	...
	for(var i=0;i<12;i++){
		var el=document.getElementById(bl4[i]);
		if(a4[i][1]=="black")
...
<% if(pl[0].equals(user)){
		for(int i=5;i<9;i++)
			out.print("<input type='radio' class='rad' name='block' value='"+i+"'><span>block"+i+"</span>");
		out.print("<br>");
		for(int i=9;i<13;i++)
			out.print("<input type='radio' class='rad' name='block' value='"+i+"'><span>block"+i+"</span>");
	}
